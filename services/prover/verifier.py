...
class Verifier:
    ...
    def lean_verifier(self, formal_proof: str) -> bool:
        with open("prompts/lean_import.lean") as f:
            lean_import = f.read()

        scheduler_input = [
            dict(code=lean_import + formal_proof, ast=False, tactics=False)
        ]
...
if __name__ == "__main__":
    ...
    # formal_proof = v.formalize(question, current_state, step)

    # print(f"Formal proof: {formal_proof}")

    formal_proof = "theorem test:  (16 ∣ 78 + 82) ∧ (16 ∣ 79 + 81) := by sorry"
    
    print(f"Lean Verifier Result: {v.lean_verifier(formal_proof)}")
