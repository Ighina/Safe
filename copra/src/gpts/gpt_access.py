...
class GptAccess(object):
    gpt_model_info = {
        "gpt-3.5-turbo": {
            ...
            "max_token_per_prompt": int(
                3.75 * 2**10
            ),  # less than 4k because additional tokens are added at times
        },
        "gpt-4": {
            ...
            "max_token_per_prompt": int(
                7.75 * 2**10
            ),  # less than 8k because additional tokens are added at times
        },
        "gpt-4-0314": {
            "token_limit_per_min": 20000,
            "request_limit_per_min": 100,
        ...
        "gpt-4-1106-preview": {
            ...
            "max_token_per_prompt": int(
                1.2 * 10**5
            ),  # less than 128k because additional tokens are added at times
        },
        "gpt-4o": {
        ...
        "codellama/CodeLlama-7b-Instruct-hf": {
            ...
            "max_token_per_prompt": int(
                13.75 * 2**10
            ),  # less than 16k because additional tokens are added at times
        },
    ...
    def __init__(
        ...
        self._load_secret()
        self.models_supported = client.models.list().data
        self.models_supported_name = [model.id for model in self.models_supported]
        if model_name is not None:
            assert (
                model_name in self.models_supported_name
    ...
    def complete_prompt(
        ...
        resp = [
            (obj.text, sum(obj.logprobs.token_logprobs)) for obj in response.choices
        ]
    ...
    def complete_chat(
        ...
        usage_dict = {
            ...
            "reason": response.choices[-1].finish_reason
            if len(response.choices) > 0
            else "stop",
        }
